...
def fill_unknown_ages(dataset):
    ...
    dataset["Age"] = dataset["Age"].fillna(dataset["age_med"])
    del dataset["age_med"]
    return dataset
...
def fill_unknown_fares(dataset):
    ...
    dataset["Fare"] = dataset["Fare"].fillna(dataset["fare_med"])
...
def train_model(dataset):
    ...
    pred_fn = tf.compat.v1.estimator.inputs.pandas_input_fn(
        x=X_val, batch_size=len(X_val), shuffle=False
    )
...
pred_fn = tf.estimator.inputs.pandas_input_fn(
    x=df_test_clean, batch_size=len(df_test_clean), shuffle=False
)
predictions = list(model.predict(input_fn=pred_fn))
final_predictions = []
for pred in predictions:
...
test_submission = pd.DataFrame(
    {"PassengerID": df_test.PassengerId, "Survived": final_predictions}
)
...
X_pca = pd.get_dummies(
    df_pca.drop(["Survived"], axis=1), columns=["Sex", "Embarked", "Title"]
)
